...
- Профіль користувача (аватар), блокування акаунтів адміном
- **Підтримка:** звернення користувачів, відповіді адміна, закриття з причиною
- **Сповіщення:** колокольчик у шапці (відповідь підтримки, rank-up, новини)
- **Новини:** публічна стрічка + адмін CRUD, сповіщення при публікації
- FAQ, Community (статичний контент), User Agreement при реєстрації
- Багатомовність **uk/en**: переклади в БД, `npm run seed-translations`, **i18next**
...
|--------|--------|----------|
| Реєстрація / вхід, JWT | Реалізовано | `bcryptjs`, `jsonwebtoken`; User Agreement на клієнті |
...
| Stealth economy (regen, блок при 0, mock-відновлення) | Реалізовано | Маскування **+50%** (cap 100); модалка з TopBar і при 0 |
| Типи завдань (code / choice / phishing / sentence) | Реалізовано | `AnswerValidator` |
| MITRE: синхронізація з GitHub CTI | Реалізовано | `POST /api/mitre/sync` або `/api/admin/mitre/sync` |
| MITRE: матриця, модальне вікно, пошук за ID | Реалізовано | `SkillMatrixPage`, `mitreSearch.ts` |
| Таблиця лідерів | Реалізовано | `GET /users/leaderboard`, `LeaderboardPage` |
| Сторінка рангів | Реалізовано | `/ranks`, посилання з rank-up сповіщень |
| Підтримка (користувач + адмін) | Реалізовано | Ліміт 3 звернення/день; закриття з причиною |
| Сповіщення | Реалізовано | Bell у TopBar, polling 60 с, i18n на клієнті |
| Новини | Реалізовано | `/news`, `/admin/news`, сповіщення при publish |
| Профіль, аватар | Реалізовано | `PUT /users/me/avatar`, crop на клієнті |
| Блокування користувачів (admin) | Реалізовано | `PATCH /admin/users/:id/block` |
| Переклади: seed, namespaces, API | Реалізовано | `seed-translations.ts`, `translations` routes |
| i18next на клієнті | Реалізовано | `preferredLocale`, перемикач мови |
| RBAC (`USER` / `ADMIN`) | Реалізовано | `requireAdmin`; роль у БД, не в JWT |
| FAQ / Community | Реалізовано | Статичні сторінки з i18n-ключами |
| `UserStats.completedLevels` | **Частково** | Поле є; орієнтир — `UserProgress` |
| Окремий компонент `StealthBar` | **Не використовується** | Stealth у `TopBar`; `StealthDepletedModal` у `Layout` |
| Платний тариф Stealth | **Заглушка** | Alert у модалці, без API оплати |
| AST-валідація коду | **Спрощено** | Підрядок / PowerShell; не повний AST |
...
│   ├── src/
│   │   ├── components/
│   │   │   ├── layout/       # Layout, Sidebar, TopBar, Footer, NotificationsBell, LanguageSwitcher
│   │   │   ├── game/         # GameLayout, StealthDepletedModal, ContextPanel, WorkArea, …
│   │   │   ├── tasks/        # CodeEditor, TacticalChoice, PhishingConstructor, …
│   │   │   ├── mitre/        # MitreTechniqueBadge, MitreTechniqueModal
│   │   │   └── profile/      # UserAvatar, crop modal
│   │   ├── pages/            # Login, Missions, SkillMatrix, Leaderboard, Ranks, FAQ,
│   │   │                     # Community, Support, News, Profile, Settings, Admin*
│   │   ├── store/            # authStore, gameStore, sidebarStore
│   │   ├── services/         # api.ts
│   │   ├── utils/            # rank, mitreSearch, notificationText, supportTicketText
│   │   ├── i18n/
│   │   └── App.tsx
├── server/
│   ├── src/
│   │   ├── routes/           # auth, missions, levels, users, mitre, translations,
│   │   │                     # admin, support, notifications, news
│   │   ├── services/         # levelService, stealthService, supportService,
│   │   │                     # notificationService, newsService, mitreSyncService
│   │   ├── middleware/       # auth, requireAdmin, ownership, rate limit
│   │   ├── validators/
│   │   └── data/missions/
│   ├── prisma/               # schema, migrations, seed.ts, seed-translations.ts
│   └── uploads/avatars/
└── shared/
```
...
| `isBlocked` | Boolean | Блокування акаунта адміном |
| `blockedAt`, `blockedReason` | DateTime?, String? | Метадані блокування |
| `avatarUrl` | String? | Шлях до аватара (`/uploads/avatars/…`) |
| `preferredLocale` | String? | `uk` / `en` для контенту новин і UI |
| `xp`, `rank`, `stealth` | — | Застарілі дублікати на `users`; актуальні значення в `UserStats` + sync stealth |
...
### Модель **SupportTicket** (`support_tickets`)

| Поле | Опис |
|------|------|
| `subject`, `message` | Тема (3–200) і перше повідомлення (10–5000) |
| `status` | `OPEN` \| `ANSWERED` \| `CLOSED` |
| `closeReason` | `ANSWERED` \| `DECLINED` \| `CUSTOM` (при закритті адміном) |
| `closeReasonText` | Текст для `CUSTOM` (3–500 символів) |
| `closedAt` | Час закриття |

### Модель **SupportMessage** (`support_messages`)

Повідомлення в thread: `body`, `isStaffReply`, `authorId`. Staff-відповіді редагує/видаляє лише автор-адмін (якщо ticket не `CLOSED`).

### Модель **Notification** (`notifications`)

| Поле | Опис |
|------|------|
| `type` | `SUPPORT_REPLY` \| `SYSTEM` (rank-up) \| `NEWS` |
| `title`, `body` | i18n-ключі або legacy-текст; body може містити ID для резолву на клієнті |
| `link` | Маршрут (`/support`, `/ranks`, `/news/:id`) |
| `supportMessageId`, `newsPostId` | FK з **ON DELETE CASCADE** |

### Модель **NewsPost** (`news_posts`)

Двомовні `titleUk`/`titleEn`, `bodyUk`/`bodyEn`, `isPublished`, `publishedAt`, `authorId`.

### Міграції, MITRE sync, seed і переклади
...
**Переклади UI** (окремо від seed місій):
...
Скрипт `prisma/seed-translations.ts` upsert-ить ключі `uk`/`en` для namespaces (`ui`, `auth`, `skillMatrix`, …).
...
- **GET** `/users/me` **JWT** — профіль поточного користувача (xp, rank, stealth, avatarUrl, role; passive regen)
- **PUT** `/users/me/locale` **JWT** — зберегти `preferredLocale` (`uk` \| `en`)
...
- **GET** `/users/leaderboard` **JWT** — таблиця лідерів (username, xp, rank, avatar)
...
- **GET** `/support/tickets/limit` — ліміт звернень на день (3/UTC-день)
- **GET** `/support/tickets` — список власних звернень
- **GET** `/support/tickets/:id` — деталі + thread повідомлень
- **POST** `/support/tickets` — створити звернення (`subject` 3–200, `message` 10–5000); **429** при перевищенні денного ліміту

### Сповіщення (`/notifications`, JWT)

- **GET** `/notifications/unread-count` — кількість непрочитаних
- **GET** `/notifications` — останні сповіщення (limit 5)
- **PATCH** `/notifications/read-all` — позначити всі прочитаними
- **PATCH** `/notifications/:id/read` — позначити одне прочитаним

### Новини (`/news`, JWT)

- **GET** `/news` — опубліковані пости (локаль з `preferredLocale`)
- **GET** `/news/:id` — один опублікований пост

### Адмін (`/admin`, JWT + ADMIN)

- **GET** `/admin/users` — список користувачів
- **PATCH** `/admin/users/:id/block` — блок / розблок (`reason` опційно)
- **GET** `/admin/mitre/stats` — покриття перекладів MITRE
- **POST** `/admin/mitre/sync` — синхронізація MITRE CTI
- **GET** `/admin/support/tickets` — усі звернення
- **GET** `/admin/support/tickets/:id` — деталі звернення
- **POST** `/admin/support/tickets/:id/reply` — staff-відповідь (+ сповіщення користувачу)
- **POST** `/admin/support/tickets/:id/close` — закрити без відповіді (`reason`: `ANSWERED` \| `DECLINED` \| `CUSTOM`, `reasonText` для CUSTOM)
- **PATCH** `/admin/support/messages/:messageId` — редагувати **власну** staff-відповідь
- **DELETE** `/admin/support/messages/:messageId` — видалити **власну** staff-відповідь
- **GET** `/admin/news` — усі пости (чернетки + published)
- **POST** `/admin/news` — створити пост
- **PATCH** `/admin/news/:id` — оновити; при першій публікації — сповіщення всім незаблокованим
- **DELETE** `/admin/news/:id` — видалити пост
...
**Блок при 0:** submit не приймається; клієнт показує `StealthDepletedModal`.
...
### Підтримка (`supportService.ts`)

- **Ліміт:** не більше **3** нових звернень на користувача за UTC-день (`GET /support/tickets/limit`).
- **Створення:** перше повідомлення зберігається в `SupportTicket` і як `SupportMessage` користувача.
- **Staff-відповідь:** `POST /admin/support/tickets/:id/reply` → статус `ANSWERED`, сповіщення `SUPPORT_REPLY` з `link: /support`.
- **Закриття без відповіді:** `POST /admin/support/tickets/:id/close` → `CLOSED`, `closeReason` / `closeReasonText`; користувач бачить причину в UI.
- **Редагування/видалення:** лише **автор** staff-повідомлення; ticket не `CLOSED`.

### Сповіщення (`notificationService.ts`)

| Тип | Коли створюється | `link` |
|-----|------------------|--------|
| `SUPPORT_REPLY` | Staff відповів на звернення | `/support` |
| `SYSTEM` | Rank-up після submit | `/ranks` |
| `NEWS` | Перша публікація поста | `/news/:id` |

Текст у БД — i18n-ключі; клієнт резолвить через `notificationText.ts`. Видалення `SupportMessage` / `NewsPost` каскадно видаляє пов’язані сповіщення.

### Новини (`newsService.ts`)

- Публічні `GET /news*` — лише `isPublished: true`; локаль з `preferredLocale` (`uk` / `en`).
- Адмін CRUD через `/admin/news`; при **першій** публікації — масове сповіщення `NEWS` усім незаблокованим користувачам.
...
| Шлях | Опис |
|------|------|
| `/login` | Вхід / реєстрація (з чекбоксом User Agreement) |
| `/agreement` | Текст угоди користувача |
| `/missions` | Список місій |
| `/missions/:missionId/assignments` | Список завдань місії |
| `/missions/:missionId/assignments/:assignmentId` | Ігровий екран (`GameLayout`) |
| `/skill-matrix` | Матриця MITRE (`?technique=T1593` — відкрити модалку) |
| `/leaderboard` | Таблиця лідерів |
| `/ranks` | Кар’єрні ранги та діапазони XP |
| `/faq` | FAQ |
| `/community` | Спільнота (статичні теми) |
| `/support` | Підтримка: нове звернення + «Мої звернення» (акордеон) |
| `/news`, `/news/:newsId` | Новини; permalink копіюється в буфер |
| `/profile` | Профіль, аватар |
| `/admin/tickets` | **ADMIN** — звернення користувачів |
| `/admin/news` | **ADMIN** — CRUD новин |
| `/settings` | **ADMIN** — користувачі, блокування, MITRE sync/stats |
...
**SkillMatrixPage** — групування за тактиками; пошук: для запитів на кшталт `T1578` / `1578.002` — лише за **ID** техніки (без false positive з описів); інакше — текстовий пошук (назва, опис, тактика); фільтри освоєних; **MitreTechniqueModal**; збереження розгорнутих тактик у `localStorage`.

**SupportPage** — форма звернення (ліміт 3/день), акордеон «Мої звернення», відображення staff-відповідей і причини закриття.

**NewsPage** — акордеон новин; «Копіювати посилання» → буфер + URL `/news/:id`.

**AdminTicketsPage** — список звернень, відповідь, редагування/видалення власних відповідей, **закриття** з причиною (`ANSWERED` / `DECLINED` / custom).

**AdminNewsPage** — створення/редагування/публікація двомовних постів.

**SettingsPage** — **лише ADMIN:** список користувачів, блокування з причиною, MITRE stats/sync (з підтвердженням і re-check ролі).

**NotificationsBell** — dropdown у TopBar; оновлення при відкритті + кожні **60 с**; текст через `notificationText.ts` (i18n-ключі + legacy fallback).

**Footer** — copyright, посилання на новини та угоду.
...
| Mock «маскування» | **+STEALTH_MASKING_RESTORE** (cap 100) |
...
- **Skill Matrix** візуалізує всі техніки з БД і перетин із пройденими ID з `GET /users/me/stats`.
...
9. **Переклади UI:** `cd server && npm run seed-translations`
10. **Frontend:** `cd client && npm run dev` (порт **5173**, проксі `/api` → backend)
11. Відкрити `http://localhost:5173`, увійти: **admin@cybertactics.test** / **admin123**
