...
**Клієнт-серверна SPA + REST API monolith** (monorepo). Frontend — component-based SPA (React, Zustand, React Router); backend — шарова архітектура Express (`routes` → `middleware` → `services` → Prisma → PostgreSQL). Детальніше — розділ «Архітектура системи» у [`documentation.md`](docs/documentation.md).
...
Детальна схема, security gates і налаштування secrets — у [docs/ci-cd-pipeline.md](docs/ci-cd-pipeline.md).
