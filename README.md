...
**Клієнт-серверна SPA + REST API monolith** (monorepo). Frontend — component-based SPA (React, Zustand, React Router); backend — шарова архітектура Express (`routes` → `middleware` → `services` → Prisma → PostgreSQL). Детальніше — розділ «Архітектура системи» у [`DOCUMENTATION.md`](DOCUMENTATION.md).
