...
function useSettingsPage(t: TFunction, isEn: boolean, currentUserId: string | undefined) {
...
function SettingsUsersTable({
  ...
  isEn: boolean;
  t: TFunction;
  onBlockRequest: (user: AdminUserSummary) => void;
}>) {
  return (
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {users.map((user) => {
              ...
              return (
                <tr key={user.id} className="border-b border-cyber-border/60 last:border-b-0">
                  <td className="px-3 py-3">
                    <div className="font-medium text-gray-100">{user.username}</div>
                    <div className="text-xs text-gray-500">{user.email}</div>
                  </td>
                  <td className="px-3 py-3 text-gray-300">{user.role}</td>
                  <td className="px-3 py-3 text-right font-mono text-cyber-primary">{user.xp}</td>
                  <td className="px-3 py-3">
                    {user.isBlocked ? (
...
function SettingsMitreSection({
  ...
  mitreStats: MitreAdminStats | null;
  syncing: boolean;
  ...
  const getLocaleTitle = (locale: 'uk' | 'en'): string => {
    if (locale === 'uk') {
      return t('settingsMitreUkTitle', {
        ns: 'ui',
        defaultValue: isEn ? 'Ukrainian (visible in UI)' : 'Українська (як у UI)',
      });
    }
    return t('settingsMitreEnTitle', {
      ns: 'ui',
    ...
    <section className="cyber-panel border border-cyber-border rounded-lg p-4 sm:p-6 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          {syncing && (
            <span className="text-sm text-yellow-400" role="status" aria-live="polite">
              {t('settingsMitreSyncInProgress', {
                ns: 'ui',
                defaultValue: isEn
          ...
          <button
            ...
            className="px-4 py-2 rounded border border-cyber-primary text-cyber-primary text-sm uppercase tracking-wide disabled:opacity-50 disabled:cursor-not-allowed hover:bg-cyber-primary/10 transition-colors"
          >
            {syncing
      ...
      {mitreStats && (
        <div className="space-y-4 text-sm">
          <div className="rounded border border-cyber-border p-3 max-w-xs">
            ...
            <div className="font-mono text-xl text-cyber-primary">{mitreStats.totalTechniques}</div>
          </div>
            ...
            <div key={locale} className="space-y-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="rounded border border-green-500/30 p-3">
                  ...
                  <div className="font-mono text-xl text-green-400">{mitreStats[locale].full}</div>
                </div>
                ...
                <div className="rounded border border-red-500/30 p-3">
                  ...
                  <div className="font-mono text-xl text-red-400">{mitreStats[locale].none}</div>
                </div>
              </div>
            </div>
          ))}
...
export default function SettingsPage() {
  ...
  const {
    ...
    handleSyncCancel,
    handleSyncConfirm,
  } = useSettingsPage(t, isEn, currentUserId);
