...
18. [Перевірка безпеки](#перевірка-безпеки)
...
**Порядок важливий:** спочатку `POST /api/mitre/sync`, потім `yarn seed`. Рівні в `levels` посилаються на `mitre_techniques` (FK). Якщо seed запустити до sync, місії створяться, але рівні — ні; `GET /api/missions/:id/levels` поверне 404.
...
- **GET** `/users/:id/progress` **JWT** — прогрес по рівнях (completed, attempts, lastAnswer); лише власний `id`
- **GET** `/users/:id/stats` **JWT** — XP, rank, Stealth, освоєні MITRE-техніки (TopBar, Skill Matrix)
...
- **POST** `/mitre/sync` — синхронізація технік з MITRE CTI у PostgreSQL (**до** seed рівнів)

### Переклади

- **GET** `/translations/languages` — список мов для перемикача (uk, en)
- **GET** `/translations?locale=&namespace=` — переклади одного namespace для i18next
- **GET** `/translations/namespaces?locale=&namespaces=` — кілька namespaces одним запитом (старт клієнта)
- **POST** `/translations` — upsert одного рядка перекладу (admin/seed)
- **POST** `/translations/bulk` — масовий upsert (`seed-translations`)

### Координатори

- **GET** `/handlers/random/:group` — випадковий handler для групи місії (legacy API; у UI Context прибрано)

**Примітки:** при Stealth ≤ 0 submit повертає `stealthDepleted: true`. Поле `correctAnswer` у відповіді submit **не повертається**. Аватари після upload: `/uploads/avatars/{userId}.jpg`.
...
1. **shared:** `cd shared && yarn install && yarn build`
2. **`.env`:** у каталозі `server/` скопіювати `.env.example` → `.env`
3. **PostgreSQL:** `cd server && docker compose up -d`
4. **Залежності:** `cd server && yarn install`; `cd client && yarn install`
5. **Міграції:** `cd server && yarn db:migrate`
6. **Backend:** `cd server && yarn dev` (порт **3001**)
...
8. **Seed:** `cd server && yarn seed` (тестовий користувач + місії/рівні)
9. **Frontend:** `cd client && yarn dev` (порт **5173**, проксі `/api` → backend)
...
1. Збірка: `shared` → `yarn build`; `server` → `yarn build`; `client` → `yarn build`.
2. Налаштування змінних середовища на сервері (`.env` за зразком `.env.example`).
3. PostgreSQL + міграції → запуск API → `POST /api/mitre/sync` → `yarn seed`.
4. Запуск API: `cd server && yarn start`.
...
Розділ описує, **як підтвердити безпеку** CyberTactics для дипломної роботи. Детальний план робіт — **тиждень 6** у [PLAN.md](./PLAN.md).
...
**Версія документації:** 2.2 (українська, розширена)  
**Оновлено:** 2026-05-23
