...
| Роль    | Права                                                                                                                                 |
|---------|---------------------------------------------------------------------------------------------------------------------------------------|
| `USER`  | Місії, профіль, progress/stats, Skill Matrix, leaderboard, support (власні звернення), notifications, новини (read)                  |
| `ADMIN` | Усі права USER + MITRE sync, переклади, керування користувачами (block), admin support, admin news CRUD                              |
...
До admin endpointʼів належать усі маршрути з префіксом **`/api/admin/*`**, а також write-операції MITRE і translations:

**MITRE і переклади**

* `POST /api/mitre/sync` (дублікат: `POST /api/admin/mitre/sync`)
...
* `PATCH /api/admin/support/messages/:messageId`
...
**MITRE (admin read/stats)**

* `GET /api/admin/mitre/stats`
...
| USER A → `GET /api/support/tickets/:id_B` | `404` (чужий ticket)         |
| USER A → `PATCH /api/notifications/:id_B/read` | `404`                      |
| Blocked user → будь-який JWT endpoint | `403 Account blocked`              |

---

### 12. Підтримка користувачів (support)

Усі маршрути **`/api/support/*`** вимагають JWT.

Користувач (`USER`) повинен мати доступ **лише до власних** звернень:

* `GET /api/support/tickets` — список за `userId` з JWT;
* `GET /api/support/tickets/:id` — деталі лише якщо `ticket.userId === userId`; інакше `404`.

Створення звернення:

* `POST /api/support/tickets` — валідація `subject` (3–200), `message` (10–5000);
* **ліміт 3 звернення на UTC-день** на користувача; при перевищенні — `429`.

Адміністративні операції — лише **`/api/admin/support/*`** (JWT + ADMIN):

* перегляд усіх звернень;
* staff-відповідь, закриття з причиною (`ANSWERED` \| `DECLINED` \| `CUSTOM`; для `CUSTOM` — `reasonText` мін. 3 символи);
* редагування / видалення **лише власних** staff-повідомлень (`authorId === adminId`).

Користувач не повинен мати можливості змінювати статус закриття або редагувати чужі повідомлення через user API.

---

### 13. Сповіщення (notifications)

Усі маршрути **`/api/notifications/*`** вимагають JWT.

* `GET /api/notifications`, `GET /api/notifications/unread-count` — лише записи з `userId` з JWT;
* `PATCH /api/notifications/read-all` — позначає прочитаними **лише власні** сповіщення;
* `PATCH /api/notifications/:id/read` — `404`, якщо сповіщення не належить поточному користувачу.

Сповіщення створюються **на сервері** (відповідь підтримки, rank-up, публікація новини). Клієнт не повинен мати API для довільного створення notification records.
...
* `GET /api/news`, `GET /api/news/:id` — JWT; лише **опубліковані** пости;
* локаль контенту — з `preferredLocale` користувача (fallback `uk`).

**Адмін (write) — `/api/admin/news/*`, JWT + ADMIN:**

* CRUD постів (чернетки + published);
* при **першій публікації** сервер створює сповіщення всім **незаблокованим** користувачам (не через клієнт).

Користувач `USER` не повинен створювати, редагувати або видаляти новини.

---

### 15. Блокування облікових записів (block)

`PATCH /api/admin/users/:id/block` — JWT + ADMIN.

Вимоги:

* тіло запиту валідується (`blocked: boolean`, `reason` опційно);
* **заборонено** блокувати власний акаунт (`400`);
* **заборонено** блокувати іншого `ADMIN` (`400`);
* заблокований користувач отримує `403 Account blocked` при login і на **будь-якому** захищеному endpoint (перевірка в `authenticate` middleware).

Блокування не повинно виконуватись через публічні або user-level API.
...
| Secure Admin Operations | `/api/admin/*`, MITRE sync, translations write захищені JWT + ADMIN.                                                       | Захищає системні дані, support, news і користувачів від несанкціонованої зміни.  |
